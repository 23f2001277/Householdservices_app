...
def signup():
    if request.method == "POST":
        ...
        new_usr = User_Info(email=uname, password=pwd,full_name=full_name, location=location, pin_code=pin_code, role=1)  # Assign role
...
def signup2():
    if request.method == "POST":
        ...
        new_prof = Prof_Info(email=uname, password=pwd, full_name=full_name, service_name=service_name, experience=experience, location=location, pin_code=pin_code) 
